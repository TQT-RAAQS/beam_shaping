...
import numpy as np
from toolkits.configs import Addresses

styles=[Addresses.mpl_style]
...
def getStylishFigureAxesWithTotalCount(num: int, nrows = None, ncols = None, **args):
    assert nrows is not None or ncols is not None, "Either nrows or ncols must be provided"
    ...
    fig, ax = getStylishFigureAxes(nrows, ncols, **args)
    if type(ax) == np.ndarray:
        for i in range(num, len(ax)):
